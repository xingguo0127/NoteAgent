...
# Lock to ensure only one request processes at a time (prevents Git race conditions)
...
async def process_screenshot(request: ProcessRequest) -> ProcessResponse:
    ...
    Note: Uses a lock to ensure requests are processed sequentially,
    preventing Git race conditions when multiple requests arrive simultaneously.

    Args:
        request: ProcessRequest with app_name, image_base64, and url

    Returns:
        ProcessResponse with status and extracted information
    """
    # Decode base64 image first (before acquiring lock)
    try:
